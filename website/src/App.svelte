<script>
  ...
  const docs = {
    ...
    tutorial: "推荐路径：表达式 -> let/let mut -> 比较/布尔逻辑/控制流 -> fn -> struct 字面量/字段访问 -> 标量 match -> enum -> check/run -> Playground/REPL。",
    api: "Stage 0 API 覆盖 Int、String、Bool、module/import、fn、let/let mut、赋值、比较、布尔逻辑、return、if/while、struct 字面量、字段访问、标量 match、enum 和 std 命名空间占位。",
...
fn main() -> Int {
  ...
  function replApi() {
    return "Zeta Stage 0 API\nInt/String/Bool\nmodule/import/fn/let/let mut/assignment/comparison/boolean logic/return/if/while/struct literal/field access/scalar match/enum\nstd: 标准库命名空间占位，当前示例 import std.io;";
  }
