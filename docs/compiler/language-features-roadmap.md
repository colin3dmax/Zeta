...
| **P3** | **Closure**(`\|x\| e` + 捕获) | 大 | **✅ 完成**(3a 语言层 9 用例 + 3b native 闭包转换 8 用例;fn ptr + heap env,差分对齐解释器) |
| **P4** | **Generics**(`<T>` 参数多态) | 大 | **✅ 完成**(4a 语言层 9 用例 + 4b native 按需单态化 9 用例;含 transitive 泛型调泛型) |
...
- **P3 Closure ✅**:**3a**(commit 2bb7bef)parser lambda + fn 类型 + runtime 闭包值(捕获快照)+ 间接调用,解释器 oracle。**3b**(commit 16204db)native 闭包转换——lambda 提升为顶层函数(显式 env 参数)+ 堆 env struct,闭包值 =(fn ptr, env);精确自由变量分析(collect_free_loads);返回类型用 infer_ztype 纯类型推导;lower_indirect_call 解包派发。
- **P4 Generics ✅**:**4a**(commit 66558c2)parser `<T>` + typecheck 类型参数 + 调用点 unify 推断/substitute;解释器透明(动态类型,无需单态化)。**4b**(commit 5717e6f)native 按需单态化:调用点按实参 ZType 特化并缓存(id$Int),codegen 的 let 从 RHS 推断使特化体无需类型替换;transitive 泛型调泛型支持。
