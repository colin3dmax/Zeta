...
| **P3** | **Closure**(`\|x\| e` + 捕获) | 大 | **3a 语言层 ✅**(lambda + 捕获快照 + fn 类型 + 间接调用,解释器 oracle,9 用例);**3b native 闭包转换待做** |
...
- **P3 Closure**:**3a ✅**(commit 2bb7bef)parser lambda + fn 类型 + runtime 闭包值(捕获快照)+ 间接调用,解释器 oracle。**3b 待做**:native 闭包转换——把 lambda 提升为顶层函数(显式 env 参数)+ 堆分配 env struct,闭包值 = (fn ptr, env);自由变量分析(3a 解释器用全量 locals 快照偷懒,native 必须精确算自由变量)。
