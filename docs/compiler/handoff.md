...
> 最新提交:`b767789`(trait 切片①② —— trait/impl 语法 + 具体类型 UFCS 派发;本地 main,**尚未 push**)。
...
| 类型系统 | ~80% | 泛型单态化齐全;**trait/impl + 具体类型 UFCS 派发已完成**(切片①②);**缺泛型约束**(切片③:`f<T: Show>`)|
...
1. **trait / 接口系统**(进行中):切片①(前端语法/AST/dump,commit 9636add)+ 切片②(具体类型 UFCS 派发,commit b767789)**已完成**。**剩切片③:泛型约束 + 单态化派发** —— `fn f<T: Show>(x:T){ show(x) }`,即在泛型函数体内调用 trait 方法(真多态),需在 native 单态化时按具体类型实参解析派发(`lower_generic_call` 内对 trait 方法调用按已知 subst 的 ZType 路由),并在 typecheck 校验类型参数满足约束。设计要点见下「trait 实现脉络」。完成后即可做 HashMap(需 hash/eq trait 派发)。
