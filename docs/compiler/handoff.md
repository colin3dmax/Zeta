...
- **`std.strings` 源码注入模块**(完成,纯 Zeta):`import std.strings;` 注入 `src/std/strings.zeta`(基于 std.core 字符串内置组合)。**纯 Zeta ⇒ 像用户代码一样 lower ⇒ 三后端天然一致、零 codegen/runtime intrinsic、fixpoint 安全**(arena_frontend 不 import ⇒ 不注入)。已含:`string_split`(契约:`"a,,b"→["a","","b"]`、`""→[""]`、无/空分隔符→整串)、`string_join`(split 逆)、`string_replace`(= split+join,空 from 恒等)、`string_starts_with`/`string_ends_with`(空前/后缀恒真;**注意**:这俩名 arena_frontend 自有同名 fn,但因 import-gated 注入而不撞——这正是源码模块比全局 intrinsic 安全之处)、`string_trim_start`/`string_trim_end`(用 `ascii_is_whitespace` 内置 ⇒ 与 `string_trim` 内置空白集精确一致:space/\t/\n/\r/FF;依赖 `&&` 短路避免越界读,all-whitespace 用例自验证短路在两后端都成立)。接入照 std.collections 模式只改三处:`std_api`(`STANDARD_IMPORTS` 项 + `is_std_strings_import`)+ `std_prelude::inject` + 新 `.zeta`(resolver 经 `is_standard_import` 自动放行)。14 个差分测试 `tests/codegen_strings.rs` + ASan 干净。**经验:补 stdlib 广度优先用源码 std 模块(只改三处、无须动 codegen、自动三后端一致),而非新 intrinsic;`string_replace=split+join` 这类组合尤其适合纯 Zeta。**
- **块体闭包 `|params| { stmts; return e; }`**(完成):闭包体可含语句块(局部 `let`、控制流、显式 `return`),与单表达式 `|x| e` 并存;按值捕获不变。**设计**:AST 加 `LambdaBody { Expr(Box<Expr>) | Block(Vec<Stmt>) }`(`parser` 中闭合 `|` 后紧跟 `{` ⇒ 块,否则表达式;`|x| Foo{..}` 不歧义因 `{` 跟在类型名后)。**MIR 层统一**:两形态都塌成 `MirExpr::Lambda { body: Vec<MirStmt> }`(expr 体 ⇒ `[return e]`),lifted function 三后端统一发射(codegen `lower_stmts`+fallthrough `unreachable`;解释器 `ClosureBody::MirBlock`/`AstBlock` 跑 stmts 取 Return)。**返回类型推断**:typecheck/mir-verify/codegen 各有 block 扫描器(累积 `let` 入 scope,取首个 `return` 的类型;codegen `block_return_ztype` + `block_has_return` 容错)。**自由变量分析**:codegen 新增 `collect_free_loads_stmts`(按 let/loop/match 绑定作用域)。**parity**:arena_frontend 自身无闭包字面量 ⇒ fixpoint 不受影响;但 stage1-parity 把含 lambda 的程序喂给两前端比 MIR dump,故 dump 对**单 `[return e]` 体**特判为旧表达式格式(两前端一致),仅真多语句块用新格式。5 个差分测试 `tests/codegen_block_closure.rs`(局部+捕获/控制流/循环/返回 String/多捕获)+ ASan 干净(托管捕获+托管返回 3000 轮无 double-free)。**坑:改 MirExpr::Lambda 表示后,stage1-parity 的 MIR dump 会与未改的 arena 前端发散——靠 dump 特判保留旧格式解决,不必回灌 arena。**
- **trait 默认方法**(完成):`trait T { fn m(self: Self) -> R { <default> } }` —— impl 省略该方法则继承默认(`Self`→target)。**实现**:`TraitMethod` 加 `default_body: Option<Vec<Stmt>>`(parser 中方法签名后 `{` ⇒ 默认体,否则 `;`);`desugar::flatten_impls` 建 trait 名→methods 表,对每个 impl **未覆盖**的默认方法,用与 impl 方法相同的 mangle(`dispatch_name`+`subst_self`+prepend impl 泛型)合成 `m$Base` 自由函数。默认体可调用 self 上的其它 trait 方法(mangle 后经 UFCS 派发)。**无须新增 codegen/typecheck**:合成发生在 desugar,`<T:Show>` 约束检查只需 `m$Base` 存在(现已由默认合成提供)。arena_frontend 无 trait ⇒ fixpoint 安全。3 个差分测试 `tests/codegen_trait_default.rs`(覆盖/继承/默认调必需方法/经 `<T:Metric>` 约束用)。
**剩余可选项(语言核心已很扎实)**:trait 关联类型、更多 stdlib 广度(照 std.strings 续写)、块体闭包回灌 arena_frontend(当前靠 dump 特判维持 parity,非必须)。
...
| 核心语言 | ~93% | 标量/数组/元组/struct/enum/闭包(参数可推断 + **块体 `\|x\| { .. }`**)、泛型单态化、`?`、**方法语法 `x.f()`** + **运算符重载**;match 含 string 模式 + **守卫 `pat if c`** |
...
| 类型系统 | ~91% | 泛型单态化 + trait/impl 完整 + `<T:Show>` 约束 + **trait 默认方法**;缺:关联类型、多 trait 对象 |
| 标准库 | ~55% | 字符串(escape 全/to_upper/lower/trim)/数组/整数工具 + **stdout print/println** + 基础文件 IO + 泛型数组 + **std.collections(HashMap/HashSet)** + **std.strings(split/join/replace/starts_with/ends_with/trim_start/trim_end)**;无网络 |
