...
| **Stage2(Zeta 自带 codegen)** | slice 1 标量 + 2 struct + 3 IntArray + 4 string ✅ | `arena_frontend.zeta` 的 `compile(src,"llvm")` 走 MIR arena 发**LLVM IR 文本**(Rust 端不参与 codegen);clang 编 + C driver 链 + 跑,结果逐一对齐 `run_mir`。类型子系统 `Tcx`;named `%S` 聚合;IntArray=`%zarr{len,ptr}` 堆 buffer + **绑定点深拷贝**;String=`%zstr{len,ptr}` 不可变(不拷),字面量→private global const(跨函数累积),`string_len/byte_at/concat/byte_slice/int_to_string(snprintf)/ascii_is_*` 内联 + `==/!=` memcmp。门禁 `tests/selfhost_llvm.rs`(31 用例:标量 + struct + array + string 全谱) |
