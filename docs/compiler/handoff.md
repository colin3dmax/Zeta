...
| **Stage2(Zeta 自带 codegen)** | slice 1 标量 + 2 struct + 3 IntArray ✅ | `arena_frontend.zeta` 的 `compile(src,"llvm")` 走 MIR arena 发**LLVM IR 文本**(Rust 端不参与 codegen);clang 编 + C driver 链 + 跑,结果逐一对齐 `run_mir`。类型子系统 `Tcx`(struct 布局/函数返回/局部作用域);named `%S` 聚合(insertvalue/extractvalue/GEP,值语义);IntArray=`%zarr{len,ptr}` 堆 buffer,**绑定点深拷贝**值语义,`int_array_empty/push` 内联 malloc+memcpy。门禁 `tests/selfhost_llvm.rs`(22 用例:标量全谱 + struct + array 字面量/index 读写/len/push/值拷贝独立) |
