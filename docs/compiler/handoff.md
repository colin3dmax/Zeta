...
> 最新提交:`c60cd75`(trait 全完成 + stdlib int_pow/string_to_int;本地 main,**尚未 push**)。本会话:trait 三切片 + stdlib 两内置,均过 fixpoint 4/4 + 全套件。
...
| 标准库 | ~35% | 字符串/数组/整数工具(+int_pow/string_to_int)+ 基础 IO;**无 Map/Set、无网络** |
...
2. **标准库继续扩**(低风险,马上可用):**已补 int_pow/string_to_int(commit c60cd75)**;待补 `string_split`/`string_to_upper`/`string_to_lower` 等(注意 §5.2/5.3,codegen 循环类内置用 alloca+基本块,镜像 `gen_string_to_int` 范式)。**`HashMap`/`Set` 现在可做**(trait 已完成,用 hash/eq trait + 泛型容器函数)。
