...
> 最新提交:`28e056a`(已 push 至 c60cd75;之后泛型容器栈未 push)。本会话:trait 三切片 + stdlib 两内置 + **泛型容器全栈(Array<T> 类型 → array_push/repeat → 泛型 HashMap<K,V>)**,均过 fixpoint 4/4 + 全套件。
...
| 标准库 | ~40% | 字符串/数组/整数工具(+int_pow/string_to_int)+ 基础 IO + **泛型数组 Array<T>/array_push/array_repeat** + **泛型 HashMap<K,V>(Zeta 库)**;无 Set/网络 |
...
1. ~~**trait / 接口系统**~~ **全部完成**(切片①9636add / ②b767789 / ③c346da6):trait/impl 语法 + 具体类型 UFCS 派发 + 泛型多态(`f<T>(x){show(x)}` 经单态化自然派发)+ `<T: Show>` 约束校验。**HashMap 现在可做**(用 `hash`/`eq` trait + 泛型容器函数 `hashmap_get<K: Hash + Eq, V>`,单态化派发已能跑)。可选增强:trait 默认方法、关联类型。
2. **标准库继续扩**(低风险,马上可用):**已补 int_pow/string_to_int(c60cd75)+ 泛型数组 Array<T>/array_push/array_repeat(6237fa6)+ 泛型 HashMap<K,V>(28e056a,testdata/generic_hashmap.zeta)**。待补:`Set`(照 HashMap 删 value)、把 HashMap 提为可 import 的 std 模块、`string_split`/`string_to_upper` 等(codegen 循环类内置镜像 `gen_string_to_int`)。**泛型容器要点**:自底向上单态化下无参 `array_empty()` 不可行,创建走 `array_repeat(seed,n)`(元素从 seed 推断);`Array<T>` 贯穿 typecheck/mir/codegen 的类型解析 + unify_ztype/resolve_generic_ztype 的 Array 分支。
